...
# To resolve the error of https://github.com/olabodeolushi/icw-app-mod/issues/68. The cause of the error is Powershell by default uses TLS 1.0 to connect to website, but website security requires TLS 1.2. You can change this behavior with running any of the below command to use all protocols. You can also specify single protocol.
...
Write-Host "Downloading icw-app-mod from GitHub" -ForegroundColor Green
New-Item -ItemType directory -Path C:\MCW
while((Get-ChildItem -Directory C:\MCW | Measure-Object).Count -eq 0 )
{
    (New-Object System.Net.WebClient).DownloadFile("https://github.com/olabodeolushi/icw-app-mod/zipball/$branchName", 'C:\MCW.zip')
...
$item = get-item "C:\MCW\*"
Rename-Item $item -NewName "icw-app-mod-$branchName"

# Replace SQL Connection String
$item = "C:\MCW\icw-app-mod-$branchName"
...
$argument = "-File `"C:\MCW\icw-app-mod-$branchName\Hands-on-lab\lab-files\ARM-template\webvm-logon-install.ps1`""
